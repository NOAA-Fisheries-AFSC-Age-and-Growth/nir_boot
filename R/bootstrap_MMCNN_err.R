...
#Metrics calculated against "known" scenario
{
  data_known <- read.csv(paste0(wd,"/sims_known/",j,"/input.csv"))
  
  train_known <- data_known[data_known$sample == 'training', ]$final_age
  test_known <- data_known[data_known$sample == 'test', ]$final_age
  
  # Convert vectors to data frames
  y_tr_df_known <- data.frame(train = train_known, pred = y_pr_transformed)
  
  # Reset index (in R, this is typically done by ensuring the data is a data frame)
  # We can add the file column directly to the data frame
  y_tr_df_known$file <- f_train
  ...
  # Convert vectors to data frames
  y_test_df_known <- data.frame(train = test_known, pred = y_pred_transformed)
  
  # Reset index (in R, this is typically done by ensuring the data is a data frame)
  # We can add the file column directly to the data frame
  y_test_df_known$file <- f_test
  ...
  # Save the plot
  ggsave(filename = "./Output/Figures/TestSet_Known.png", plot = plot, width = 12, height = 12, units = "in", dpi = 300)
  
  # Print the plot
  print(plot)
  
  
  data_known <- y_test_df_known %>%
    mutate(
      Mean = (train + pred) / 2,
      Difference = train - pred,
      Mean_diff = mean(Difference),
  ...
  r_squared_tr_known <- r2_score(train_known, y_pr_transformed)
  rmse_tr_known <- rmse_manual(train_known, y_pr_transformed)
  
  r_squared_known <- r2_score(test_known, y_pred_transformed)
